...
import { Wallet, Plus, ArrowUpRight, ArrowDownRight, Send } from 'lucide-react';
...
const WalletPage = () => {
  ...
  // Fetch active payment gateways for top-up instructions
  ...
  const sendMoneyNumber = selectedGw?.credentials?.phone_number;
  const sendMoneyAccountType = selectedGw?.credentials?.account_type;
  ...
  return (
    <div className="space-y-6">
      <h2 className="font-heading text-2xl font-bold">Wallet</h2>

      {/* Balance Card */}
      <div className="bg-gradient-to-br from-primary to-primary/80 text-primary-foreground rounded-2xl p-6 md:p-8">
        <div className="flex items-center gap-3 mb-2">
          <Wallet className="h-6 w-6" />
          <span className="text-sm font-medium opacity-80">Available Balance</span>
        </div>
        <p className="font-heading text-4xl md:text-5xl font-bold">৳{balance.toLocaleString()}</p>
        <div className="mt-6 flex gap-3 flex-wrap">
          {/* Add Funds */}
          <Dialog open={addOpen} onOpenChange={setAddOpen}>
            <DialogTrigger asChild>
              <Button variant="secondary" className="gap-2"><Plus className="h-4 w-4" /> Add Funds</Button>
            </DialogTrigger>
            <DialogContent>
              ...
              <div className="space-y-4 py-2">
                ...
                {/* Show payment number */}
                {topupMethod && sendMoneyNumber && (
                  <div className="bg-muted rounded-lg p-3 text-center">
                    <p className="text-sm text-muted-foreground">Send money to:</p>
                    <p className="text-xl font-bold text-primary">{sendMoneyNumber}</p>
                    <p className="text-xs text-muted-foreground">{selectedGw?.display_name} ({sendMoneyAccountType || 'Personal'})</p>
                  </div>
                )}

                <div className="space-y-2">
                  <Label>Transaction ID *</Label>
                  <Input placeholder="Enter Transaction ID after sending" value={topupTxId} onChange={(e) => setTopupTxId(e.target.value)} />
                </div>

                <div className="bg-muted/50 rounded-lg p-3 text-xs text-muted-foreground">
                  ⏳ Please wait 10-15 minutes after submitting. Your balance will be loaded after admin verification.
                </div>
          ...
          <Dialog open={withdrawOpen} onOpenChange={setWithdrawOpen}>
            <DialogTrigger asChild>
              <Button variant="secondary" className="gap-2" disabled={balance <= 0}><Send className="h-4 w-4" /> Withdraw</Button>
            </DialogTrigger>
            <DialogContent>
