// Validates a hand-authored subject question file and emits SQL INSERTs.
...
const existingRaw = fs.readFileSync(
  'C:\\Users\\Mahbub\\AppData\\Local\\Temp\\claude\\d--Desktop-ots-new\\3df15fab-9fd3-4460-bb5c-24283746c76f\\scratchpad\\existing_questions_clean.txt',
  'utf8'
);
...
questions.forEach((q, i) => {
  ...
  if (!['basic', 'intermediate', 'advanced'].includes(q.difficulty)) errors.push(`[${i}] bad difficulty: ${q.q}`);
  if (!q.exp || q.exp.length < 15) errors.push(`[${i}] explanation missing/too short: ${q.q}`);

  const points = q.difficulty === 'basic' ? 1 : q.difficulty === 'intermediate' ? 2 : 3;
  rows.push({ ...q, points });
});
...
const byDiff = rows.reduce((acc, r) => ((acc[r.difficulty] = (acc[r.difficulty] || 0) + 1), acc), {});
