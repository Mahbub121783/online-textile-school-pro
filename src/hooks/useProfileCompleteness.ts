...
export const useProfileCompleteness = (profile: any) => {
  const fields: ProfileField[] = useMemo(() => {
    if (!profile) return [];
    return [
      { key: 'avatar_url', label: 'Profile Picture', weight: 15, completed: !!profile.avatar_url },
      { key: 'full_name', label: 'Full Name', weight: 15, completed: !!profile.full_name },
      { key: 'phone', label: 'Phone Number', weight: 15, completed: !!profile.phone },
      { key: 'blood_group', label: 'Blood Group', weight: 10, completed: !!profile.blood_group },
      { key: 'university', label: 'University/Institution', weight: 10, completed: !!profile.university },
      { key: 'batch', label: 'Batch', weight: 10, completed: !!profile.batch },
      { key: 'district', label: 'District', weight: 5, completed: !!profile.district },
      { key: 'professional_role', label: 'Current Role', weight: 10, completed: !!profile.professional_role },
